 ...
 * @author ThanhNhan
 */
public class StatisticalClass extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StatisticalClass</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StatisticalClass at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
            ...
            if (a != null) {
                ...
                switch (type) {
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        request.setAttribute("total", BookReceiptDB.listBookReceiptAllStudent().size());
                        request.setAttribute("list", BookReceiptDB.listBookReceiptAllStudent());
                        BookReceiptDB.statistical(BookReceiptDB.listBookReceiptAllStudent());
                        request.getRequestDispatcher("StatisticalClass.jsp").include(request, response);
                        break;
                }
            } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (from.compareTo(to) > 0) {
                ...
                if (grade != -1) {
                    for (BookReceipt b : BookReceiptDB.listBookReceiptGrade(grade)) {
                    ...
                    for (BookReceipt b : BookReceiptDB.listBookReceiptAllStudent()) {
                        if (from.compareTo((Date) d.parse(b.getBorrowDate().substring(9, b.getBorrowDate().length()))) <= 0 && to.compareTo((Date) d.parse(b.getBorrowDate().substring(9, b.getBorrowDate().length()))) >= 0) {
                            list.add(b);
                        }
                    }
                ...
                request.setAttribute("G" + grade, "selected");
